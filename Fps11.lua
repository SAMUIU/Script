...
-- Corrige la cámara sin bloquear toques, permite usar botones de interfaz (Shift Lock, habilidades, minimapa) y evita que el personaje se quede estático
    ...
    if player then
        ...
        pcall(function()
            if isGamepadConnected then
                ...
                -- Desactivar scripts de control de cámara personalizados en el juego
                local playerScripts = player:WaitForChild("PlayerScripts")
                for _, script in pairs(playerScripts:GetChildren()) do
                    if script:IsA("LocalScript") and (script.Name:lower():find("camera") or script.Name:lower():find("control")) then
