...
-- Corrige el problema de la cámara con joystick virtual en móvil y evita desplazamiento de la interfaz
...
local function lowerResolution()
    ...
    if player then
        ...
        -- Evitar usar IgnoreGuiInset para no desplazar la interfaz
    ...
    if player then
        ...
        pcall(function()
            if isGamepadConnected then
                ...
                -- Bloquear control de cámara por joystick virtual en móvil
                UserInputService.InputBegan:Connect(function(input, gameProcessedEvent)
                    if gameProcessedEvent then return end
                    if input.UserInputType == Enum.UserInputType.Touch then
                        -- Asumir que el joystick de cámara está en la mitad derecha de la pantalla
                        if input.Position and input.Position.X > camera.ViewportSize.X / 2 then
