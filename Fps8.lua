...
-- Corrige el problema de la cámara con joystick virtual en móvil y permite usar botones de interfaz
    ...
    if player then
        ...
        pcall(function()
            if isGamepadConnected then
                ...
                -- Bloquear control de cámara por joystick virtual en móvil (solo parte superior derecha)
                UserInputService.InputBegan:Connect(function(input, gameProcessedEvent)
                    ...
                    if input.UserInputType == Enum.UserInputType.Touch then
                        if input.Position then
                            ...
                            -- Solo bloquear toques en la parte superior derecha (joystick de cámara)
                            if screenPos.X > viewportSize.X * 0.5 and screenPos.Y < viewportSize.Y * 0.5 then
...
local function optimizeGame()
    pcall(function()
        ...
        optimizeLighting()
        stabilizeCamera()
        print("Optimización de FPS completada: texturas desactivadas, partículas reducidas, resolución bajada, FPS desbloqueados, cielo gris añadido, cámara estabilizada, botones de interfaz habilitados.")
    end)
end
