...
-- Función para estabilizar la cámara
...
local function resetMovementAfterSkill()
    ...
    if humanoid and rootPart then
        pcall(function()
            -- Restablecer estados del Humanoid
            humanoid.PlatformStand = false
            humanoid.Sit = false
            humanoid.WalkSpeed = math.max(humanoid.WalkSpeed, 16)
            humanoid.JumpPower = math.max(humanoid.JumpPower, 50)
            
            -- Desanclar RootPart para evitar inmovilización
            if rootPart.Anchored then
                rootPart.Anchored = false
            end
            
            -- Forzar movimiento si está estático
            if humanoid.MoveDirection.Magnitude == 0 and rootPart.Velocity.Magnitude < 1 then
                -- Aplicar un pequeño impulso para "desatascar"
...
RunService.Heartbeat:Connect(function(deltaTime)
    -- Reducir frecuencia de verificación para optimizar en Android
    lastCheckTime = lastCheckTime + deltaTime
    if lastCheckTime >= 0.2 then -- Verificar cada 0.2 segundos
        pcall(function()
            resetMovementAfterSkill()
        end)
        lastCheckTime = 0
...
UserInputService.InputEnded:Connect(function(input, gameProcessedEvent)
    if gameProcessedEvent and input.UserInputType == Enum.UserInputType.Touch then
        task.spawn(function()
            task.wait(0.3) -- Tiempo reducido para respuesta rápida
            pcall(function()
                resetMovementAfterSkill()
            end)
        end)
